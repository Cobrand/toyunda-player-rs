...
impl Display for Text2D {
    fn draw(self, displayer: &mut Displayer) {
        ...
        let (text_pos_x, text_pos_y) = real_position((window_width, window_height),
                                                     self.pos,
                                                     self.anchor,
        ...
        for text_element in self.text.iter() {
            ...
            let syllable_rect = text_element.blit(font_set,
                ...
                match displayer.lyrics_logo {
                    Some(ref texture) => {
                        displayer.renderer.copy(&texture,
                                                None,
                                                Some(Rect::new(syllable_center_x -
            ...
            let (w, _): (u32, u32) =
                font_set.get_regular_font().size_of(text_element.text.as_str()).unwrap();
