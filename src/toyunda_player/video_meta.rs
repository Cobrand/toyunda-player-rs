...
impl VideoMeta {
    pub fn from_yaml<P:AsRef<Path>>(path:P) -> Result<VideoMeta,String> { 
        ...
        match ::std::fs::File::open(path) {
            Ok(yaml_file) => {
                serde_yaml::from_reader(yaml_file).map_err(|err|{
                    format!("Error when parsing .yaml file : {}",err)
                })
            },
            Err(e) => {
                error!("Error when loading yaml file '{}' : {}",path.display(),e);
                Err(format!("Error when loading yaml file '{}'; file could not be opened",path.display()))
            }
        }
    }
    ...
    pub fn new<P:AsRef<Path>>(path:P) -> Result<VideoMeta,String> {
        ...
        match path.extension() {
            None => {
                Err(format!("Error when trying to load file '{}', file has no extension",path.display()))
            },
            Some(s) if s == "yaml" => {
                VideoMeta::from_yaml(path).map(|v_m| {
                    v_m.fix_paths(path)
                })
            },
            Some(s) if s == "avi" || s == "mp4"  => {
                ...
                if yaml_file.exists() {
                    VideoMeta::from_yaml(yaml_file).map(|v_m| {
                        v_m.fix_paths(path)
                    })
                } else {
                    Ok(VideoMeta {
            ...
            Some(ext) => {
                Err(format!("Unrecognized extension '{}'",ext.to_str().unwrap_or("[NON-UTF-8 SEQ]")))
            }
        }
    }
