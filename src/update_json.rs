    ...
    if let Some(path) = args.value_of("JSON_FILE") {
        ...
        if (json_path.is_file() && yaml_path.is_file()) {
            ...
            match (json_file,yaml_file) {
                (Ok(json_file),Ok(yaml_file)) => {
                    ...
                    match (video_meta,subs) {
                        (Ok(video_meta),Ok(mut subs)) => {
                            ...
                            let mut json_file = File::create(&json_path).expect("Can't open json file for writing");
                            if let Err(e) = serde_json::to_writer_pretty(&mut json_file,&subs) {
                                println!("Some error occured while generating new subtitles : {:?}",e);
                                false
                            } else {
                                true
                            }
                ...
                (Err(e),_) => {
                    println!("file `{}` couldn't be opened : {:?}",json_path.display(),e);
                    false
                },
                (_,Err(e)) => {
                    println!("file `{}` couldn't be opened : {:?}",yaml_path.display(),e);
                    false
                }
            }
        } else {
