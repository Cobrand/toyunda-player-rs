...
impl<'a> ToyundaPlayer<'a> {
                ...
                if let Some(ref sub) = self.subtitles {
                    ...
                    let thread = ::std::thread::spawn(move || {
                        match ::std::fs::File::create(&json_file_path) {
                            ...
                            Err(e) => {
                                error!("Failed to write-open subtitles file {} : {:?}",
    ...
    pub fn start(&mut self, params: StartupParameters) -> Result<()> {
        ...
        if enable_manager {
            let manager = Manager::new(&*format!("{}:{}",
                                                 params.manager_listen_address,
                                                 params.manager_listen_port),
                                       Arc::downgrade(&self.state),
                                       params.lookup_directories,
                                       self.songs_history.as_ref());
            match manager {
                Ok(manager) => {
                    self.manager = Some(manager);
                }
                Err(e) => {
                    error!("Error when initializing manager : '{}'", e);
                }
            }
        }
    ...
    pub fn on_load_media(&mut self) -> Result<ToyundaAction> {
        ...
        if let Err(e) = res {
            if self.mode == ToyundaMode::KaraokeMode {
                if let &PlayingState::Playing(ref video_meta) = &self.state
                    .write()
                    .unwrap()
                    .playing_state {
    ...
    pub fn load_media_from_video_meta(&mut self, video_meta: VideoMeta) -> Result<ToyundaAction> {
        let tmp_video_path = video_meta.video_path.to_str().map(|s| String::from(s));
    ...
    pub fn import_cur_file_subtitles(&mut self) -> Result<()> {
        let duration: u32 = (self.mpv.get_property::<f64>("duration").unwrap_or(0.0) *
                             1000.0) as u32;
        let fps: f64 = self.get_file_fps();
        let (json_path, lyr_path, frm_path) = match &self.state.read().unwrap().playing_state {
            &PlayingState::Idle => {
                return Err(Error::Text(String::from("Error when reloading subtitles : no file \
                                                     is playing !")))
            }
            &PlayingState::Playing(ref video_meta) => {
        ...
        if let Some((first_sentence, second_sentence_opt)) = subs.credit_sentences() {
            ...
            if percent >= 1.0 - CREDITS_TRANSITION_TIME {
                ...
                if let Some(second_sentence) = second_sentence_opt {
                    ...
                    color.alpha = (inner_percent(percent, 1.0, 1.0 - CREDITS_TRANSITION_TIME) *
                                   192.0) as u8;
                }
                ...
                if let Some(second_sentence) = second_sentence_opt {
                    string = second_sentence;
                    color.alpha = (inner_percent(percent, 0.5, 0.5 + CREDITS_TRANSITION_TIME) *
                                   192.0) as u8;
                }
            } else if percent >= 0.5 - CREDITS_TRANSITION_TIME {
            ...
            if !string.is_empty() {
                ...
                overlay_frame.text_units.push(TextUnit {
                    ...
                    pos: (PosX::FromLeftPercent(0.02), PosY::FromBottomPercent(0.03)),
                    anchor: (0.0, 1.0),
                });
            }
        };
    ...
    pub fn messages_as_overlay_frame(&mut self) -> OverlayFrame {
        ...
        let graphic_messages = graphic_messages.into_iter()
            .filter(|m| !is_karaoke_mode || m.category == Category::Announcement);
        ...
        let mut overlay_frame = OverlayFrame { text_units: vec![] };
        ...
        let display_params: DisplayParams = match (self.mpv_cache.cached_width(),
            ...
            (Some(w), Some(h)) => {
                ...
                if mpv_aspect_ratio < screen_aspect_ratio {
                    final_w = ((mpv_aspect_ratio / screen_aspect_ratio) * (width as f64)) as u32;
                    final_h = height;
                    offset_y = 0;
                    offset_x = ((width - final_w) / 2) as i32;
                } else {
                    final_h = ((screen_aspect_ratio / mpv_aspect_ratio) * (height as f64)) as u32;
                    final_w = width;
                    offset_x = 0;
    ...
    pub fn main_loop(&mut self, sdl_context: &Sdl) -> Result<()> {
        ...
        'main: loop {
            ...
            self.mpv
                ...
                .expect("failed to draw video frame with mpv");
            ...
            while let Some(event) = self.mpv.wait_event(0.0) {
                match event {
                    ...
                    MpvEvent::EndFile(Err(MPV_ERROR_LOADING_FAILED)) => {
                        match &self.state.read().unwrap().playing_state {
                            &PlayingState::Playing(ref video_meta) => {
                                error!("Failed to load video stream `{}` current song will be \
                                        skipped",
                                       video_meta.video_path.display());
                            }
                            _ => {}
    ...
    pub fn handle_event(&mut self,
        ...
        match event {
            ...
            Event::KeyDown { keycode: Some(Keycode::S), .. } if is_ctrl_pressed &&
                                                                mode == NormalMode => {
                self.execute_command(Command::Stop)
            }
            Event::KeyDown { keycode: Some(Keycode::N), .. } if is_ctrl_pressed &&
                                                                mode == NormalMode => {
                self.execute_command(Command::PlayNext)
            }
