...
impl StartupOptions {
    pub fn from_args<'a>(arg_matches:ArgMatches<'a>) -> Result<StartupOptions,String> {
        Ok(StartupOptions {
            manager_listen_address: match arg_matches.value_of("manager_listen_address") {
                Some(listen_address_str) => Some(try!(listen_address_str.parse::<IpAddr>().map_err(|e| format!("{}",e)))),
                None => None,
            },
            ...
            volume: match arg_matches.value_of("volume") {
                Some(volume_str) => Some(try!(volume_str.parse::<f64>().map_err(|e| format!("{}",e)))),
                None => None,
            },
            ...
            mode: match (arg_matches.is_present("edit_mode"),arg_matches.is_present("karaoke_mode")) {
    ...
    pub fn to_params(self) -> StartupParameters {
        StartupParameters {
            manager_listen_address:self.manager_listen_address.unwrap_or(IpAddr::V4(Ipv4Addr::new(0,0,0,0))),
